...
import glob
import yaml
from extract_3libai_config import get_args
...
def extract_result(args, extract_func):
    ...
    logs_list_faban = glob.glob(os.path.join(args.faban_log, "*/*/output.log"))
    logs_list_faban = sorted(logs_list_faban)

    throughput_final_result_dict = {}
    markdown_table_header = """

|      | [{}](https://github.com/Oneflow-Inc/oneflow/tree/{}) | [{}](https://github.com/Oneflow-Inc/oneflow/tree/{}) | [{}](https://github.com/Oneflow-Inc/oneflow/tree/{})                                                     |
| ---- | ------------------------------------------------------------ | ------------------------------------------------------------ | ------------------------------------------------------------ |"""
    markdown_table_body = """
| {case_name}   | [{libai_memory}]({libai_yaml}) MiB/[{libai_samples}]({libai_log}) samples/s                                 | [{libai_memory_compare}]({libai_yaml_compare}) MiB/[{libai_samples_compare}]({libai_log_compare}) samples/s | [{libai_memory_faban}]({libai_yaml_faban}) MiB/[{libai_samples_faban}]({libai_log_faban}) samples/s |"""

    tmp_markdown_table_header = markdown_table_header.format(
        logs_list[0].split("/")[-4], logs_list[0].split("/")[-4], logs_list_compare[0].split("/")[-4], logs_list_compare[0].split("/")[-4], logs_list_faban[0].split("/")[-4], logs_list_faban[0].split("/")[-4]
    )
    ...
    for l in logs_list_faban:
        ...
        if case_header not in throughput_final_result_dict.keys():
            throughput_final_result_dict[case_header] = {}
        throughput_final_result_dict[case_header]["case_name_faban"] = case_header
        throughput_final_result_dict[case_header]["libai_memory_faban"] = libai_result_dict[
            "memory"
        ]
        throughput_final_result_dict[case_header][
            "libai_yaml_faban"
